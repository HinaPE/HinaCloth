...
int main() {
    ...
    if (r.status != Status::Ok) {
        std::printf("ex03: create failed\n");
        return 1;
    }
    Solver* s = r.value;

    // Enable attachment
    const char* att = "attachment";
    Command c_en{CommandTag::EnableOperator, &att, sizeof(att)};
    push_command(s, c_en);
    // Pin top row
    RegionPayload pinTop{"inv_mass", 0u, nx, {0.0f, 0, 0}};
    Command c_pin{CommandTag::SetFieldRegion, &pinTop, sizeof(pinTop)};
    push_command(s, c_pin);
    // Set a vertical stripe at mid column with weight=0.7 and target shifted +X
    uint32_t mid = nx / 2;
    for (uint32_t j = 0; j < ny; ++j) {
        uint32_t id = vid(mid, j, nx);
        RegionPayload w{"attach_w", id, 1u, {0.7f, 0, 0}};
        Command cw{CommandTag::SetFieldRegion, &w, sizeof(w)};
        push_command(s, cw);
        float tx = pos[3u * id + 0] + 0.15f, ty = pos[3u * id + 1], tz = pos[3u * id + 2];
        RegionPayload t{"attach_target", id, 1u, {tx, ty, tz}};
        Command ct{CommandTag::SetFieldRegion, &t, sizeof(t)};
        push_command(s, ct);
    }
