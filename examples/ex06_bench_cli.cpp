...
static Args parse_args(int ac, char** av) {
    Args a{};
    for (int i = 1; i < ac; ++i) {
        if (std::strncmp(av[i], "--backend=", 10) == 0)
            a.backend = parse_backend(av[i] + 10);
        else if (std::strncmp(av[i], "--layout=", 9) == 0)
            a.layout = parse_layout(av[i] + 9);
        else if (std::strncmp(av[i], "--threads=", 10) == 0)
            a.threads = std::atoi(av[i] + 10);
        else if (std::strncmp(av[i], "--nx=", 5) == 0)
            a.nx = (uint32_t) std::atoi(av[i] + 5);
        else if (std::strncmp(av[i], "--ny=", 5) == 0)
            a.ny = (uint32_t) std::atoi(av[i] + 5);
        else if (std::strncmp(av[i], "--frames=", 9) == 0)
            a.frames = std::atoi(av[i] + 9);
        else if (std::strncmp(av[i], "--iters=", 8) == 0)
            a.iterations = std::atoi(av[i] + 8);
        else if (std::strncmp(av[i], "--subs=", 7) == 0)
            a.substeps = std::atoi(av[i] + 7);
        else if (std::strcmp(av[i], "--sweep") == 0)
            a.sweep = true;
    }
    return a;
}
...
static const char* layout_name(DataLayout l) {
    switch (l) {
    case DataLayout::SoA: return "soa";
    case DataLayout::Blocked: return "blocked";
    default: return "auto";
    }
}
