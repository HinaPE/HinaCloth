...
int main(){
    uint32_t nx=64, ny=48; float dx=0.025f; std::vector<float> pos, vel; std::vector<uint32_t> edges; make_grid(nx,ny,dx,pos,vel,edges);
    ...
    auto r = create(bd); if (r.status!=Status::Ok){ std::printf("ex05: create failed\n"); return 1; }
    Solver* s = r.value;
    auto qc = query_chosen(s); if (qc.status==Status::Ok){ auto ch=qc.value; std::printf("ex05: backend=%d layout=%d threads=%d\n", (int)ch.backend, (int)ch.layout, ch.threads);}
    const float dt = 1.0f/60.0f; for(int f=0; f<90; ++f) step(s, dt);
