...
static void make_grid(uint32_t nx, uint32_t ny, float dx, std::vector<float>& pos, std::vector<float>& vel, std::vector<uint32_t>& edges, std::vector<uint32_t>& bend_pairs) {
    uint32_t n = nx * ny;
    pos.resize(3u * n);
...
int main() {
    uint32_t nx = 24, ny = 16;
    float dx = 0.05f;
    std::vector<float> pos, vel;
    std::vector<uint32_t> edges, quads;
    make_grid(nx, ny, dx, pos, vel, edges, quads);
    ...
    const char* tagsd[] = {"edges"};
    FieldUse uses[]     = {{"position", true}};
    OperatorDecl opd{"distance", tagsd, 1, uses, 1, OpStage::Solve, true};
    const char* tagsb[] = {"bend_pairs"};
    OperatorDecl opb{"bending", tagsb, 1, uses, 1, OpStage::Solve, true};
    ...
    if (r.status != Status::Ok) {
        std::printf("ex04: create failed\n");
        return 1;
    }
    Solver* s = r.value;

    const char* bend = "bending";
    Command c_en{CommandTag::EnableOperator, &bend, sizeof(bend)};
    push_command(s, c_en);
    flush_commands(s, ApplyPhase::BeforeFrame);

    const float dt = 1.0f / 60.0f;
    for (int f = 0; f < 120; ++f) step(s, dt);
    TelemetryFrame tf{};
