...
int main(){
    uint32_t nx=12, ny=12; float dx=0.05f;
    std::vector<float> pos, vel; std::vector<uint32_t> edges; make_grid(nx,ny,dx,pos,vel,edges);
    ...
    // Exec: explicitly request Blocked layout to illustrate selection; threads auto; telemetry on
    Policy pol{{DataLayout::Blocked, Backend::Auto, -1, true, true}, {2, 12, 0.02f, TimeStepper::Symplectic}};
    ...
    auto r = create(bd); if (r.status != Status::Ok) { std::printf("ex01: create failed\n"); return 1; }
    Solver* s = r.value;
    auto qc = query_chosen(s);
    if (qc.status == Status::Ok){
        auto ch = qc.value;
        const char* b = ch.backend==Backend::Native?"Native":(ch.backend==Backend::AVX2?"AVX2":(ch.backend==Backend::TBB?"TBB":"Auto"));
        const char* l = ch.layout==DataLayout::SoA?"SoA":(ch.layout==DataLayout::AoS?"AoS":(ch.layout==DataLayout::Blocked?"Blocked":"Auto"));
        std::printf("ex01: chosen backend=%s layout=%s threads=%d\n", b, l, ch.threads);
    }
    const float dt = 1.0f/60.0f; for(int f=0; f<60; ++f) step(s, dt);
    TelemetryFrame tf{}; Status st_tf = telemetry_query_frame(s, &tf); (void)st_tf;
    std::printf("ex01: step_ms=%.3f residual=%.6f sub=%d it=%d\n", tf.step_ms, tf.residual_avg, tf.solve_substeps, tf.solve_iterations);
    destroy(s);
    return 0;
}
