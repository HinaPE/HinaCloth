...
int main(){
    uint32_t nx=20, ny=14; float dx=0.05f; std::vector<float> pos, vel; std::vector<uint32_t> edges; make_grid(nx,ny,dx,pos,vel,edges);
    ...
    auto r = create(bd); if (r.status!=Status::Ok){ std::printf("ex02: create failed\n"); return 1; }
    Solver* s = r.value;

    // Queue runtime commands: increase iterations, set substeps, adjust gravity, and pin left edge (inv_mass=0)
    ParamPayload itp{"iterations", 16.0f}; Command c_it{CommandTag::SetParam, &itp, sizeof(itp)}; push_command(s, c_it);
    ParamPayload sbp{"substeps", 2.0f};  Command c_sb{CommandTag::SetParam, &sbp, sizeof(sbp)}; push_command(s, c_sb);
    ParamPayload ddp{"damping",  0.02f}; Command c_dp{CommandTag::SetParam, &ddp, sizeof(ddp)}; push_command(s, c_dp);
    ParamPayload gyp{"gravity_y", -12.0f}; Command c_gy{CommandTag::SetParam, &gyp, sizeof(gyp)}; push_command(s, c_gy);
    ...
    for(uint32_t j=0;j<ny;++j){
        ...
        Command c_pin{CommandTag::SetFieldRegion, &pin, sizeof(pin)}; push_command(s, c_pin);
    }
