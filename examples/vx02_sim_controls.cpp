...
public:
    ...
    void initialize(const EngineContext& eng, const RendererCaps& caps, const FrameContext&) override {
        ...
        VkPipelineLayoutCreateInfo lci{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO}; lci.pushConstantRangeCount=1; lci.pPushConstantRanges=&pcr; VK_CHECK(vkCreatePipelineLayout(device_, &lci, nullptr, &layout_));
        ...
        pci.pNext=&rendering; pci.stageCount=2; VkPipelineShaderStageCreateInfo stages[2]{};
        ...
        pci.pStages=stages; pci.pVertexInputState=&vi; pci.pInputAssemblyState=&ia; pci.pViewportState=&vp; pci.pRasterizationState=&rs; pci.pMultisampleState=&ms; pci.pColorBlendState=&cb; pci.pDynamicState=&dyn; pci.layout=layout_;
        ...
        // Buffers
        create_host_buffer(eng, sizeof(float)*3u*(nx_*ny_), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vbuf_);
    ...
    void on_imgui(const EngineContext&, const FrameContext&) override {
        ...
        ImGui::Text("sub=%d it=%d", tf.solve_substeps, tf.solve_iterations);
        ImGui::End();
    }
    ...
    void record_graphics(VkCommandBuffer cmd, const EngineContext& eng, const FrameContext& frm) override {
        ...
        // Update vertex data from solver
        size_t nverts = (size_t)nx_ * (size_t)ny_;
        staging_.resize(3u*nverts);
        (void)copy_positions(solver_, staging_.data(), nverts, nullptr);
        // Center/scale based on XZ bbox
        ...
        Push push{}; float s = 0.9f / h; push.scale[0] = s; push.scale[1] = s; push.offset[0] = -cx * s; push.offset[1] = -cz * s;
        ...
        VkRenderingAttachmentInfo color{VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO}; color.imageView = target.view; color.imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; color.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; color.storeOp = VK_ATTACHMENT_STORE_OP_STORE; color.clearValue = clear;
